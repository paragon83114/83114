...
if [ ! -f "$diario" ]; then
    curl -s -o "$diario" https://raw.githubusercontent.com/paragon83114/termux/main/daily.md 2>/dev/null || touch "$diario"
fi
...
if [ "$1" = "eval" ]; then
    ...
    if [ "$last_entry" -gt "$last_eval" ] || [ ! -f "$HOME/.diario_last_eval" ] || [ "${2:-}" = "--force" ]; then
        ...
        opencode run -m opencode/deepseek-v4-flash-free "procesa el fichero $diario" && date +%s > "$HOME/.diario_last_eval" && echo -e "${green}OK${reset}" || { echo -e "${red}Error${reset}"; exit 1; }
    else
...
if [ "$1" = "del" ]; then
    [ ! -f "$diario" ] && { echo -e "${red}No existe${reset}"; exit 1; }
    ult=$(grep -n "### " "$diario" | tail -n 1 | cut -d: -f1)
    [ -z "$ult" ] && { echo -e "${yellow}Sin entradas${reset}"; exit 0; }
    hasta=$((ult - 1))
    [ "$hasta" -le 0 ] && { echo -e "${red}Formato inesperado${reset}"; exit 1; }
...
fecha=$(date '+%d de %b de %Y'); hora=$(date '+%H:%M')
